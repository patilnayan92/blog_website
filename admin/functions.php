...
function insert_categories(){
  ...
  if(isset($_POST['submit'])){
    ...
    $cat_name  = clean_url($_POST['cat_title']);
    $chk_url_exists=mysqli_query($connection,"select MAX(cat_name) as cat_name from categories where cat_name like '".$cat_name."%'");
      
    if(mysqli_num_rows($chk_url_exists)>0){     
      $get_pr=mysqli_fetch_array($chk_url_exists);        
      $strre=0;
      if($get_pr['cat_name']!=''){
        $strre=str_replace($cat_name,'',$get_pr['cat_name']);
      }
      if($strre=='0'){          
        $newcnt=''; 
      }elseif($strre=='') {         
    ...
    if(empty($cat_title)){
      ...
      $query= "INSERT INTO categories(cat_title,cat_name) values('$cat_title','$cat_name')";
...
function delete_categories(){
    ...
    $del_id = $_GET['delete'];
    echo $query = "DELETE FROM categories WHERE cat_id = $del_id";exit;
